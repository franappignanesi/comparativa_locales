...
export async function promotePendingReleases(mode = process.env.RELEASE_PROMOTE_MODE ?? "weekly"): Promise<{
  mode: string;
  approved: number;
  limit: number;
  ...
  if (!pendingFile?.pending?.length) {
    return { mode: normalizedMode, approved: 0, limit, totalCandidates: existing.length, broadSample: 0, strictSample: 0, minReviews, minOwners };
  }
  ...
  const promoted = approved.map((item) => ({
    ...item.candidate!,
    notes: `${item.candidate!.notes} Promovido por release automation ${normalizedMode} ${promotedAt}.`,
...
function toCandidate(input: {
  ...
  const genreTags = (input.steamSpy.genre ?? "").split(",").map((item) => item.trim()).filter(Boolean);
